...
!(async () => {
  ...
  if (ids == null) {
    $notification.post('Chưa thêm TestFlight APP_ID', 'Vui lòng thêm thủ công hoặc sử dụng liên kết TestFlight để tự động lấy', '')
  } else if (ids == '') {
    $notification.post('Đã thêm tất cả TestFlight', 'Vui lòng tắt plugin này thủ công', '')
  } else {
    ids = ids.split(',')
        ...
        if (resp.status == 404) {
          ...
          if (jsonData.data == null) {
            ...
            $httpClient.post({ url: testurl + ID + '/accept', headers: header }, function (error, resp, body) {
              ...
              $notification.post(jsonBody.data.name, 'TestFlight tham gia thành công', '')
