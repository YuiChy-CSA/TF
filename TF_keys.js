...
if (reg1.test($request.url)) {
    ...
    if ($persistentStore.read('request_id') !== null) {
      $notification.post('Thông tin TF được truy xuất', 'Truy xuất thông tin thành công, vui lòng đóng script!','')

    } else {
      $notification.post('Thông tin TF không được truy xuất','Truy xuất thông tin thất bại, vui lòng bật công tắc Mitm over HTTP2 và khởi động lại VPN và ứng dụng TestFlight!','')
    }
    $done({})
}
if (reg2.test($request.url)) {
  ...
  $notification.post("TestFlight tự động tham gia", `Đã thêm APP_ID: ${id}`, `ID hiện tại: ${appId}`);
  $done({})
}
function unique(arr) {
