...
Source: https://raw.githubusercontent.com/Yuheng0101/X/main/Tasks/AutoJoinTF.js
*/
...
const getParams = () => {
    ...
    const handler = (appId) => {
        ...
        if (!APP_IDS.includes(CACHE_APP_ID)) {
            ...
            $.msg($.name, 'Lấy tham số ứng dụng thành công', `Đã thu thập và lưu trữ ID ứng dụng: ${appId}`);
        } else {
    ...
    if (/^https:\/\/testflight\.apple\.com\/v3\/accounts\/.*\/apps$/.test(url)) {
        ...
        if (appIdMatch && appIdMatch[1]) {
            ...
            $.log('Không tìm thấy ID ứng dụng TestFlight hợp lệ');
        }
...
const TF_Check = (app_id) => {
    return new Promise((resolve, reject) => {
        $.get({ url: baseURL + app_id, headers }, (error, response, data) => {
            if (error) {
                return reject(`${app_id} Lỗi yêu cầu mạng: ${error}`);
            }
            if (response.status !== 200) {
                APP_IDS.splice(inArray(app_id), 1);
                $.setdata(APP_IDS.join(','), 'tf_app_ids');
                $.msg('Không phải liên kết TestFlight hợp lệ', '', `${app_id} đã bị xóa`);
                return reject(`${app_id} Không phải liên kết hợp lệ: Mã trạng thái ${response.status}, xóa ID ứng dụng`);
            }
            const appData = $.toObj(data);
            if (!appData) {
                return reject(`${app_id} Lỗi giải mã dữ liệu: ${data}`);
            }
            resolve(appData);
        });
    });
        ...
        $.post(
            ...
            (error, response, data) => {
                if (!error && response.status === 200) {
                    ...
                    if (!jsonBody) {
                        return reject(`${app_id} Lỗi giải mã phản hồi yêu cầu tham gia: ${data}`);
                    }
                    resolve(jsonBody);
                } else {
                    reject(`${app_id} Tham gia thất bại: ${error || `Mã trạng thái ${response.status}`}`);
                }
            }
        );
    });
...
(async () => {
    ...
    if (!noJoinExists) return $.log('Không có ID ứng dụng cần tham gia');
            ...
            for (let i = 0; i < LOON_COUNT; i++) {
                ...
                try {
                    ...
                    if (appData.data?.status === 'OPEN') {
                        $.log(`${appId}(${appData.data.app.name})`, `Có thể tham gia, đang tham gia...`);
                        const jsonBody = await TF_Join(appId);
                        $.log(`🎉Tham gia thành công`);
                        $.msg(`${jsonBody.data.name}`, 'Tham gia TestFlight thành công');
