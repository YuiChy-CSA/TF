...
!(async () => {
  ...
  if (ids == "") {
    $notify("Tất cả TF đã được thêm", "Vui lòng đóng thủ công", "");
    $done();
  } else {
    ids = ids.split(",");
    try {
    ...
    $task.fetch({ url: testurl + ID, method: "GET", headers: header }).then(
      (resp) => {
        ...
        if (resp.status == 404) {
          ...
          console.log(ID + " " + "Ứng dụng TestFlight không tồn tại và APP_ID đã bị xóa tự động");
          $notify(ID, "Ứng dụng TestFlight không tồn tại", "APP_ID đã bị xóa tự động");
          resolve();
        } else {
